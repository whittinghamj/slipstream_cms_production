...
class mag_device extends BaseClass {
    ...
    public function define_device(){
        $encode_mac = base64_encode($this->mac);
        $sql = "SELECT * FROM `mag_devices` WHERE `mac` = '$encode_mac'";
    ...
    public function define_line(){
        $sql = "SELECT * FROM `customers` WHERE `id` = '" . $this->table_columns["customer_id"] ."'";
        $query = $this->database->query($sql);
    ...
    public function require_stb($require_action){
        ...
        switch($require_action){
            case "handshake":
                ...
                //var_dump($data); - Comment Out per Asads request.
            ...
            case "set_aspect":
                if(array_key_exists("ch_id",$this->queryString)){
                    ...
                    if(empty($this->aspect)){
                        $update = "UPDATE `mag_devices` SET `aspect` = '" . json_encode(array("js" => $this->queryString["aspect"])) . "' WHERE `mag_id` = '". $this->mag_id ."'";
    ...
    public function require_itv($require_action){
        switch ($require_action) {
            ...
            case "get_genres":
                ...
                $set_stream = $this->database->query('SELECT `category_id` FROM `streams`');
                $stream_categories = array();
                foreach($set_stream as $get_streams){
                    array_push($stream_categories, $get_streams['category_id']);
                }

                foreach(array_unique($stream_categories) as $stream_category_id){
                    $set_stream_category = $this->database->query("SELECT * FROM `stream_categories` WHERE `id` = '$stream_category_id'");
                    $output["js"][] = array(
                        "id" => $set_stream_category[0]["id"],
                        "title" => $set_stream_category[0]["name"],
                        "alias" => $set_stream_category[0]["name"]
                    );
                }
                return $output;
        }
